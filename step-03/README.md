...
```bash
ansible -i step-03/hosts -m shell -a 'uname -a' host0
```
...
```bash
ansible -i step-03/hosts -m copy -a 'src=/etc/hosts dest=/tmp/' host0
```
...
know which Ubuntu version we have deployed on nodes, it's pretty easy:
    ...
    ansible -i step-03/hosts -m shell -a 'grep DISTRIB_RELEASE /etc/lsb-release' all
...
Try it out:
    ...
    ansible -i step-03/hosts -m setup host0
...
`ansible -i step-03/hosts -m setup -a'filter=ansible_memtotal_mb' all`:
